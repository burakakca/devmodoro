/**
 * General analytics service for core metrics and time-based computations.
 * This service is always loaded as it provides essential analytics functions.
 */
 ...
 * Uses local timezone instead of UTC to correctly attribute sessions
 */
 ...
 */
export const getTimeRangeConfig = (range: TimeRange): TimeRangeConfig => {
	const now = new Date();
...
export const getCurrentStreak = async (): Promise<number> => {
	...
	for (const session of sessions) {
		const dateKey = getLocalDateKey(new Date(session.startTime));
...
const computeActivityData = (
	...
	if (range === "all") {
		...
		for (const session of focusSessions) {
			...
			// Get Monday of the week (without mutating original date)
			const dayOfWeek = sessionDate.getDay();
			const daysToMonday = dayOfWeek === 0 ? -6 : 1 - dayOfWeek;
			const monday = new Date(
				sessionDate.getFullYear(),
				sessionDate.getMonth(),
				sessionDate.getDate() + daysToMonday,
			);
